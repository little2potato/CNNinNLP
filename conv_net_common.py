...
def logging_argv_info(logging_level, argv_dict):
    for key in argv_dict:
        logging_level("%s=%s"%(key, str(argv_dict[key])))

def load_data(datasets, batch_size):
    assert os.path.isfile(datasets), "not dataset found!"
    f = open(datasets)
    # train_set_x, train_set_y = ([], []), []
    # test_set_x, test_set_y = ([],[]), []
    train_set_x, train_set_y, test_set_x, test_set_y, word_idx_matrix = cPickle.load(f) #in numpy.array
    f.close()
    
    # add extra data if neccessary
    inst_num = train_set_y.shape[0]
    if inst_num % batch_size > 0:
        extra_num = batch_size - inst_num % batch_size
        logging.debug('add % extra data'%extra_num)
        perm = numpy.random.permutation(inst_num)
        indices = perm[0:extra_num]
        extra_x_data0 = train_set_x[0][indices]
        extra_x_data1 = train_set_x[1][indices]
        extra_y_data = train_set_y[indices]
        new_train_x = (numpy.append(train_set_x[0], extra_x_data0, axis=0),
                numpy.append(train_set_x[1], extra_x_data1, axis=0))
        new_train_y = numpy.append(train_set_y, extra_y_data, axis=0)
        train_set_x = new_train_x
        train_set_y = new_train_y
    x1, y = shared_dataset((train_set_x[0], train_set_y))
    x2, y = shared_dataset((train_set_x[1], train_set_y))
    t1, ty = shared_dataset((test_set_x[0], test_set_y))
    t2, ty = shared_dataset((test_set_x[1], test_set_y))
    return [(x1, x2), y, (t1, t2), ty, word_idx_matrix ]
...
class HiddenLayer(object):
    ...
    def __init__(self, rng, input, n_in, n_out, activation, W=None, b=None,
        ...
        self.L2_norm = T.sqrt((self.W ** 2).sum() + EPS)
...
class DropoutHiddenLayer(HiddenLayer):
    def __init__(self, rng, input, n_in, n_out,
        ...
        #self.L2_norm = HiddenLayer.L2_norm
...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations,use_bias=True):
        ''' layer_sizes: a list of input_layer_size, hidden_layer_sizes, output_layer_sizes'''
        ...
        self.L2_norm = numpy.sum([ layer.L2_norm for layer in self.dropout_layers ])
...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out):
        ...
        self.L2_norm = (self.logRegressionLayer.L2_norm + self.hiddenLayer.L2_norm)
